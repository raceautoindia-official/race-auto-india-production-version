...
export default function HeaderAdsAdmin() {
  ...
  const [form, setForm] = useState({ id: '', link_url: '', sort_order: '', image: null });
  ...
  const fetchAds = async () => {
    const res = await fetch('/api/admin/adspace/header-ads');
  ...
  const handleImageChange = (e) => {
    ...
    img.onload = function () {
      if (img.width !== 1200 || img.height !== 150) {
        toast.error("Image must be exactly 1200x150 pixels");
        return;
      }
  ...
  const handleDelete = async (id) => {
    ...
    const res = await fetch(`/api/admin/adspace/header-ads?id=${id}`, { method: 'DELETE' });
    if (res.ok) {
  ...
  return (
    <div className="p-4">
      <ToastContainer />
      <h4 className="mb-3">🖼️ Header Ad Manager</h4>

      <form onSubmit={handleSubmit} className="mb-4 border rounded p-3 shadow-sm bg-light">
        {form.id && <p className="text-muted">Editing ID: {form.id}</p>}
        ...
        <label className="form-label">Upload Image (1200 x 150, max 500KB)</label>
        <input
          type="file"
          accept="image/*"
          className="form-control mb-2"
        ...
        <div className="d-flex gap-2">
          <button type="submit" className="btn btn-primary" disabled={loading}>
            {loading ? 'Saving...' : form.id ? 'Update Ad' : 'Create Ad'}
          </button>
          {form.id && (
      ...
      <table className="table table-hover table-bordered table-sm">
        ...
        <tbody>
          {ads.map(ad => (
            <tr key={ad.id} className={form.id == ad.id ? 'table-warning' : ''}>
              <td>{ad.id}</td>
              <td>
                <img
              ...
              <td>
                <button
                  ...
                  onClick={() => {
                    setForm({ ...ad, image: null });
                    setPreview(`${process.env.NEXT_PUBLIC_S3_BUCKET_URL}${ad.image_url}`);
                  }}
                >
                  Edit
                </button>
